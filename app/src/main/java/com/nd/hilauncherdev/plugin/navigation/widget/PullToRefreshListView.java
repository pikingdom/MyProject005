...
public class PullToRefreshListView extends ListView {
    ...
    protected LinearLayout mBannerHeadView = null;
    ...
    /**
     * @desc 添加在下拉刷新header之后的header
     * @author linliangbin
     * @time 2017/8/24 19:23
     */
    ...
    protected void init(Context context) {
        ...
        mBannerHeadView = (LinearLayout) mInflater.inflate(R.layout.news_banner_container, null);
        addHeaderView(mBannerHeadView);

        initPullHeader();
        if (mHeadView == null) {
            mHeadView = new PullToRefreshHeader(context);
        }
     ...
     * 隐藏新闻头部的大图
     */
    public void disableBannerHeader() {

        try {
            if (mBannerHeadView != null) {
                mBannerHeadView.setVisibility(GONE);
                this.removeHeaderView(mBannerHeadView);
            }
        } catch (Exception e) {
            e.printStackTrace();
        }
    ...
    protected void onTouchMoveByList(MotionEvent event) {
        ...
        if (false == mIsRecored && 0 == this.getFirstVisiblePosition()) {
            View view = getChildAt(this.getFirstVisiblePosition());
            if (view != null && view.getTop() >= -2) {
    ...
    public LinearLayout getBannerContainer() {
        return mBannerHeadView;
    }

    public void setPullRateListent(OnPullRateListen listent) {
        mOnPullRateListen = listent;
    }
