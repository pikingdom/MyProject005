package com.nd.hilauncherdev.plugin.navigation.widget.third_party;
...
public class HeaderViewPager extends PullToRefreshView {
    ...
    public boolean dispatchTouchEvent(MotionEvent ev) {
        ...
        if (mVelocityTracker != null) {
            ...
            yVelocity = mVelocityTracker.getYVelocity();  //获取当前的滑动速度
        }
        ...
        int yDistance = (int) (currentY - mDownY);
//        Log.i("llbeing", "dispatchTouchEvent:shiftY=" + shiftY);
//        Log.i("llbeing", "HeaderViewPager:dispatchTouchEvent:" + ev.getAction() + "," + currentX + "," + currentY);
        switch (ev.getAction()) {
            //Down事件主要初始化变量
            case MotionEvent.ACTION_DOWN:
                ...
                if (isHeadTop() && mScrollable.isTop()) {
                    setPullEnable(true);
                } else {
                    setPullEnable(false);
                }
...
//                    Log.i("llbeing", "UP Velocity:" + shiftY + ",isSticked:" + isStickied() + ",isHeadTop" + isHeadTop());
                    if (yDistance <= 0 && mCurY <= ScreenUtil.dip2px(getContext(), 30) && !isHeadTop()) {
                        scrollTo(0, -mCurY);
//                        mScroller.startScroll(0,0,0,-mCurY,5000);
            ...
            case MotionEvent.ACTION_CANCEL:
                Log.i("llbeing", "HeaderViewPager:action cancel");
                recycleVelocityTracker();
                break;
            default:
                break;
            ...
            if (mDirection == DIRECTION_UP) {
                ...
                if (isStickied()) {
                    ...
                    if(onScrollListener != null){
                        onScrollListener.onFlingEnd(duration);
                    }
                ...
                if (mScrollable.isTop() || isClickHead) {
                    ...
                    if (mCurY <= minY) {
                        mScroller.abortAnimation();
                        if(onScrollListener != null){
                            onScrollListener.onFlingEnd(500);
                        }
                        return;
                    }
                }
