...
public class GZipHttpUtil {
	...
	public static String get(String url, String encoding, HashMap<String, String> paramsMap) {
		...
		try {
			String encodeUrl = utf8URLencode(buildGetURL(url, paramsMap));
	 ...
	 * @return
	 */
	public static String get(String url,HashMap<String, String> headerParamsMap){
		HttpGet request = null;
		HttpClient client = null;
		String responseStr = null;

		try {
			String encodeUrl = utf8URLencode(buildGetURL(url, null));
			request = new HttpGet(encodeUrl);
			request.setHeader("Accept-Encoding", "gzip, deflate");
			...
			client = getDefaultHttpClient();
			responseStr = client.execute(request, getResponseHandler(HTTP.UTF_8));
		} catch (Exception ex) {
			ex.printStackTrace();
		} finally {
			abortConnection(request, client);
